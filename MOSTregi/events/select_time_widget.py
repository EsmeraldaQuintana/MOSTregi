...
class SelectTimeWidget(Widget):
    ...
    def render(self, name, value, attrs=None):
        try: # try to get time values from a datetime.time object (value)
            ...
            # note:
            # The keyword "basestring" was apparently removed in Python3. From
            # what I was reading, this call was essentially just checking if
            # "value" was an instace of string or unicode. I don't think we
            # need to check for unicode in Python3 though, so I just simplified
            # this to instanceof str.
#            if isinstance(value, basestring):
        ...
        # NOTE
        # The signature for build_attrs was changed in a Django update. Apparently
        # it's an undocumented method anyway, so probably shouldn't be used...
        # I read somewhere that attrs.update does about the same thing, so I tried
        # using that instead. Not sure why we're using local_attrs instead of just
        # self.attrs though.
        # local_attrs = self.build_attrs(self, id=self.hour_field % id_)
        self.attrs.update(id=self.hour_field % id_)
